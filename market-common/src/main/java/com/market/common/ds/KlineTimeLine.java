...
public class KlineTimeLine {
    ...
    public static void main(String[] args) {
        KlineTimeLine timeline = new KlineTimeLine(TimeUnit.SECONDS.toMillis(5), 10, true);

        new Thread(() -> {
            while (true) {
                CmdResult<MarketDetailTick> rs = timeline.tick();
                if (rs.isSuccess()) {
                    try {
                        MarketDetailTick aggregate = rs.get();
                        String log = String.format("windows update: high: %s low: %s open: %s close: %s vol: %s amount: %s count: %d",
                                aggregate.getHigh().toPlainString(),
                                aggregate.getLow().toPlainString(),
                                aggregate.getOpen().toPlainString(),
                                aggregate.getClose().toPlainString(),
                                aggregate.getVol().toPlainString(),
                                aggregate.getAmount().toPlainString(),
                                aggregate.getCount());
        ...
        while (true) {
            ...
            tick.setId(System.currentTimeMillis() / 1000);
            open = open.add(BigDecimal.ONE);
            tick.setOpen(open);
            tick.setClose(open);
            tick.setCount(1);
            tick.setLow(open);
            tick.setHigh(open);
            tick.setAmount(BigDecimal.ONE);
            tick.setVol(open);
